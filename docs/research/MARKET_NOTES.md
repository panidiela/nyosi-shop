# Notes de marché Nyosi

## Marché cible immédiat (Démarrage)
- Yaoundé et Douala, Cameroun
- Estimation : 50 000–200 000 vendeurs actifs sur WhatsApp
- Cible réaliste année 1 : 200–500 vendeurs payants

## Marché à terme
- Afrique francophone subsaharienne
- Cameroun, Côte d'Ivoire, Sénégal, RDC, Burkina Faso
- Estimation TAM : 5–10 millions de vendeurs WhatsApp

## Comportements clés observés à Yaoundé

### Les vendeurs
- Postent leurs produits sur WhatsApp Status tous les matins entre 7h et 9h
- Reçoivent des commandes par message toute la journée
- Utilisent MTN MoMo et Orange Money pour recevoir les paiements
- N'ont pas de site web (trop cher, trop complexe, pas de ROI visible)
- Téléphones Android : Samsung Galaxy A, Tecno Spark, Itel — écrans 6–6.7 pouces

### Les acheteurs
- Commandent sur WhatsApp ou Facebook directement
- Paient en Mobile Money ou à la livraison (préférence MoMo pour la confiance)
- Veulent une confirmation rapide — sans réponse dans l'heure, ils vont ailleurs
- Font confiance aux vendeurs qu'ils connaissent déjà

## Pricing de référence (FCFA)

| Service de référence | Prix mensuel |
|---|---|
| Canal+ Cameroun | ~5 000 FCFA |
| Forfait internet MTN | 2 000–5 000 FCFA |
| Logiciel comptabilité local | 10 000–30 000 FCFA/an |
| **Nyosi Standard (cible)** | **5 000 FCFA/mois** |
| **Nyosi Pro (cible)** | **10 000 FCFA/mois** |

## Saisonnalité importante
- **Décembre / Noël** : pic de commandes (gâteaux, pagnes, cadeaux) → meilleure période pour lancer
- **Ramadan** : pic pour l'alimentation et les plats cuisinés
- **Rentrée scolaire** : pic pour les vêtements et uniformes
- **Saint-Valentin** : pic pour les gâteaux et cadeaux

## Risques marché
1. Confiance : "Est-ce que mon argent est en sécurité ?" → mitiger avec transparence et confirmation immédiate
2. Connexion instable : pages doivent charger vite même en 3G
3. Résistance au changement : certains vendeurs préfèrent WhatsApp → commencer avec les plus ouverts
4. Copie rapide : si ça marche, des copies vont apparaître → avantage à la vitesse d'exécution
